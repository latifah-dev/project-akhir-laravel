<script>
  Alpine.data("add", ()=>({
    ...
    const size = document.querySelector('input[name="radio"]:checked').value
      ...
      const tambah = fetch('http://127.0.0.1:8000/api/itemtransaksi', {
        method: 'POST',
        body: data,
      })
        .then(response => response.json())
                ...
                <div class="-mx-4 flex flex-wrap space-y-6 lg:space-y-0">
                  <div class="px-4 w-full lg:w-6/12"> 
                    <div class="bg-secondary-500 lg:p-4">
                      ...
                      <img :src="'http://127.0.0.1:8000/storage/images/'+products.image"  alt="" class="object-cover w-full h-full">
                      </div>
                    </div>               
                  </div>
                  <div class="px-4 w-full lg:w-6/12"> 
